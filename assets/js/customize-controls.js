/* global fpcconcordBgColors, fpcconcordColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( fpcconcordBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( fpcconcordBgColors[ context ].color ) {
						backgroundColorValue = fpcconcordBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( fpcconcordBgColors[ context ].setting ).get();
					}
					fpcconcordSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( fpcconcordBgColors ).forEach( function( context ) {
			wp.customize( fpcconcordBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					fpcconcordSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		fpcconcordSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				fpcconcordSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function fpcconcordSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = fpcconcordColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function fpcconcordSetRetineLogoVisibility( visible ) {
