/* global fpcconcordBgColors, fpcconcordPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( fpcconcordBgColors ).forEach( function( context ) {
					fpcconcordGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function fpcconcordGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'fpcconcord-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#fpcconcord-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( fpcconcordPreviewEls[ context ] ) {
				_.each( fpcconcordPreviewEls[ context ], function( items, setting ) {
	...
	$( function() {
		fpcconcordGenerateColorA11yPreviewStyles( 'content' );
		fpcconcordGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
